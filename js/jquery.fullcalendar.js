/**
 * 
 * 利用jQuery EasyUI设计的calendar日历
 * 参考jQuery EasyUI的中文官网：http://bbs.btboys.com Easyui中文社区
 */
(function ($) {
	...
	//获取当前日期
	var Today = new Date();
	var tY = Today.getFullYear(); // 当前的年份
	var tM = Today.getMonth(); // 当前的月份
	var tD = Today.getDate(); // 当前的日期
	...
	function init(target) {
		$(target).addClass('calendar').wrapInner(
			'<div class="calendar-header">' +
				...
				'<a class="calendar-year" style="margin-left:5%"></a>' + '&nbsp;' +
				'<a class="calendar-nextyear" style="margin-left:5%"></a>' + '&nbsp;' +
				'<a class="calendar-prevmonth" style="margin-left:5%"></a>' + '&nbsp;' +
				'<a class="calendar-month" style="margin-left:5%"></a>' + '&nbsp;' +
				'<a class="calendar-nextmonth" style="margin-left:5%"></a>' +
			'</div>' +
...
//		alert("showMonth.进入--->");
	...
	function show(target,fl_ag) {
				...
				if (opts.month == day[1]) {
					...
					if (opts.lunarDay) {
						...
						if (opts.solarTerms && info.solarTerms) {
							dayHtml += '<span color="#34aadc">' + info.solarTerms + '</span>';
						} else {
		...
		var now = new Date();
		var today = now.getFullYear() + '-' + (now.getMonth() + 1) + '-' + now.getDate();//当前日期
...
//			alert(current);
			t.find('td[abbr="' + current + '"]').addClass('calendar-selected').css('backgroundColor', 'rgb(129, 208, 242)');// 当前日期的颜色
		}
		...
		t.find('td').hover(
			...
			//先设置样式
			t.find('.calendar-selected').removeClass('calendar-selected').css('backgroundColor', '');
			$(this).addClass('calendar-selected').css('backgroundColor', 'rgb(129, 208, 242)');// 点击后的背景为蓝色的底
				
	        var parts = $(this).attr('abbr').split('-');//点击的时候，获取点击时的日期
            flag=true;
		    if(parts[1]<10){
				data_time=parts[0]+"-0"+parts[1];
			}else{
				data_time=parts[0]+"-"+parts[1];
			}
			if(parts[2]<10){
				data_time+="-0"+parts[2];
		    }else{
...
//		alert(opts.year+"年"+opts.month+"月");
	...
	$('head').append('<style>.fullcalendar-day {border-left: 1px solid #eee;border-top: 1px solid #eee;} '
		 + '.calendar-hover{background:rgb(52, 170, 220);}.fullcalendar-body th{padding: 5px;height:20px;}'
		 + '.fullcalendar-detail{border-radius:5px;position: absolute; z-index: 10; background-color: #34aadc; display: none; opacity: 0.8; padding: 5px; text-align: right;filter: Alpha(opacity=80);width:150px;}'
